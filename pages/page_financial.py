...
def render():
    ...
    with col1:
        ...
        fig_f.add_trace(go.Bar(x=fdi["date"], y=fdi["inflow"], name="Inflow",
            ...
            hovertemplate="<b>%{x|%b %Y}</b><br>Inflow: RM %{y:,.2f}B<extra></extra>"))
        fig_f.add_trace(go.Bar(x=fdi["date"], y=fdi["outflow"], name="Outflow",
            marker_color=COLORS["accent1"],
            hovertemplate="<b>%{x|%b %Y}</b><br>Outflow: RM %{y:,.2f}B<extra></extra>"))
        fig_f.add_trace(go.Scatter(x=fdi["date"], y=fdi["net"], mode="lines+markers",
            ...
            hovertemplate="<b>%{x|%b %Y}</b><br>Net: RM %{y:,.2f}B<extra></extra>"))
    ...
    with col2:
        ...
        for acct, lbl in BOP_ACCOUNTS.items():
            ad = bop.query(f"account=='{acct}'")
            if ad.empty: continue
            fig_b.add_trace(go.Scatter(x=ad["date"], y=ad["balance"], mode="lines+markers",
                name=lbl, marker=dict(size=4),
                hovertemplate=f"<b>{lbl}</b><br>%{{x|%b %Y}}<br>RM %{{y:,.1f}}M<extra></extra>"))
    ...
    fig_n.add_trace(go.Scatter(x=fdi["date"], y=fdi["net"], mode="lines", fill="tozeroy",
        ...
        hovertemplate="<b>%{x|%b %Y}</b><br>Net FDI: RM %{y:,.2f}B<extra></extra>"))
